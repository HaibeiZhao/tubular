function generateFastMarchingCenterlines(tubi, cntrlineOptions)
% generateFastMarchingCenterlines(tubi, cntrlineOptions)
...
startendptH5FileName = tubi.fileName.startendPt ;
fn = tubi.fileBase.name ;
...
[xyzlim, ~, ~, xyzlim_um] = tubi.getXYZLims() ;  % xyz maxima for grid formation if xx,yy,zz not supplied
...
flipy = tubi.flipy ;              % APDV coordinate system is mirrored across XZ wrt data coords           
...
outdir = tubi.dir.cntrline ;
figoutdir = fullfile(tubi.dir.cntrline, 'images') ;
...
for tt = timePoints
    ...
    if overwrite || (~exist(outname, 'file') && ~other_exist)
        ...
        try
            ...
            % A better way to plot it
            clf
            disp('Could not find path within mesh between endpoints!')
        ...
        if length(skel_tmp) > 3
            ...
            % Check it
            tubi.setTime(tt)
            amesh0 = tubi.getCurrentAlignedMesh() ;
            amesh = struct() ;
            amesh.f = fv.faces ;
            amesh.v = tubi.xyz2APDV(fv.vertices) ;
            subplot(1, 3, 1)
